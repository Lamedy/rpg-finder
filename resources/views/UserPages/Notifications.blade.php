        ...
        @if ( $notice['notice_type'] == 0)
            <div class="bg-gray-200 rounded shadow border border-black font-alegreya_medium text-xl">
                <p class="p-2 break-words">Пользователь
                    ...
                    желает вступить в вашу команду. Вы согласны?
                </p>
                ...
                <div class="flex flex-wrap justify-start px-2 gap-2 flex-1 min-w-[200px] mb-2">
                    <form class="w-full sm:w-auto" action="{{ route('not_accept_invite', ['room' => $notice->playerSession->gameSession, 'notice' => $notice]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <button type="submit"
                    ...
                    <form class="w-full sm:w-auto" action="{{ route('accept_invite', ['room' => $notice->playerSession->gameSession, 'notice' => $notice]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <button type="submit"
                                class="bg-[#2D2D2D] hover:bg-[#1a1a1a] text-white text-center rounded px-2 py-1 transition text-base sm:text-lg w-full sm:w-60">
